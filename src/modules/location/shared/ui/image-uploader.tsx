...
export function ImageUploader({
	...
	return (
		<div className={cn("w-full max-w-4xl", className)}>
				...
				<Alert variant="destructive" className="mt-5">
					...
					<AlertDescription>
						{errors.map((error, index) => (
							<p key={`${error}-${index + 1}`} className="last:mb-0">
								{error}
							</p>
						))}
					</AlertDescription>
			...
			>
				<DialogContent className="**:data-[slot=dialog-close]:text-muted-foreground **:data-[slot=dialog-close]:hover:text-foreground **:data-[slot=dialog-close]:bg-background w-full border-none bg-transparent p-0 shadow-none sm:max-w-xl **:data-[slot=dialog-close]:-end-7 **:data-[slot=dialog-close]:-top-7 **:data-[slot=dialog-close]:size-7 **:data-[slot=dialog-close]:rounded-full">
